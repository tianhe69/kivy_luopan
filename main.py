...
class Unbuffered:
    ...
    def __getattr__(self, attr):
        ...
        raise AttributeError(f"'NoneType' object has no attribute '{attr}'")

# 应用无缓冲输出（处理打包后windowed模式下stdout为None的情况）
...
def print_app_status():
    while is_running:
        ...
        elapsed = current_time - app_start_time
        print(f"[STATUS] Running for {elapsed:.2f}s, GC: {len(gc.get_objects())} objects")
        time.sleep(1)
...
if __name__ == '__main__':
    # 启动状态监控线程
    import threading
    status_thread = threading.Thread(target=print_app_status)
            ...
            BoxLayout:
                ...
                BoxLayout:
                    ...
                    TextInput:
                        ...
                        input_filter: 'float'
                        multiline: False
                        on_text_validate: root.on_compass_scale_validate(self)
                ...
                BoxLayout:
                    ...
                    TextInput:
                        ...
                        input_filter: 'int'
                        multiline: False
                        on_text_validate: root.on_rotation_change(self)
...
"""
        ...
        class CompassApp(App):
            ...
            def on_start(self):
                ...
                Clock.schedule_once(lambda dt: self.show_threshold_dialog(), 1.0)  # 延迟1秒显示
            
            def on_stop(self):
                """应用停止时调用"""
                print("罗盘控制器已关闭")
                print(f"应用运行时间: {time.time() - app_start_time:.2f}秒")
                print("应用程序停止原因: 正常退出")
        ...
        app.run()
        print("CompassApp已退出")
        print(f"应用总运行时间: {time.time() - app_start_time:.2f}秒")
    except Exception as e:
        ...
        print(f"总运行时间: {time.time() - app_start_time:.2f}秒")
