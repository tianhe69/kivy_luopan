...
class MainScreen(Screen):
    ...
    def save_image(self):
        ...
        try:
            ...
            if hasattr(self, 'displayed_image') and self.displayed_image is not None:
                # 使用image_processor的save_image方法，支持中文路径
                success = self.image_processor.save_image(save_path, self.displayed_image)
                if success:
                ...
                success = self.image_processor.save_image(save_path, self.image_processor.processed_image)
                if success:
                    print(f"图像已保存到: {save_path}")
                else:
                    print(f"图像保存失败: {save_path}")
    ...
    def on_rotation_change(self, instance):
        """旋转角度变化"""
        print(f"on_rotation_change被调用，实例: {instance}")
        try:
    ...
    def on_compass_scale_validate(self, instance=None):
        """罗盘倍数验证"""
        print(f"on_compass_scale_validate被调用，实例: {instance}")
        try:
            # 如果没有提供实例，直接从ids中获取
            if instance is None and 'compass_scale_input' in self.ids:
                instance = self.ids.compass_scale_input
            if instance:
    ...
    def on_compass_scale_apply(self, instance=None):
        """罗盘倍数应用按钮点击"""
    ...
    def _draw_compass_on_image(self, img):
        ...
        # 绘制玄空大卦罗盘（附加罗盘）
        if self.image_processor.compass_manager.show_xuankongda and 0 <= cx < img_width and 0 <= cy < img_height:
            # 使用与周天度数环相同的max_radius值
            max_radius = min(cx, cy, img_width - cx, img_height - cy)
            try:
                ...
                for line in lines_xuankongda:
                    ...
                    # 确保坐标在图像范围内
                    if 0 <= start[0] < img_width and 0 <= start[1] < img_height and 0 <= end[0] < img_width and 0 <= end[1] < img_height:
                        cv2.line(img, start, end, color, thickness)
            except Exception as e:
                print(f"绘制玄空大卦罗盘时出错: {e}")
                import traceback
                traceback.print_exc()
            ...
            try:
                ...
                for i, (x, y, label) in enumerate(texts_xuankongda):
                    try:
                        ...
                        # 确保坐标在图像范围内
                        if 0 <= int(x) < img_width and 0 <= int(y) < img_height:
                            # 绘制白色圆形背景
